...
-- 휴가 사용 이유가 '두통'인 사원, 소속 본부 조회
SELECT E.EMP_NAME, E.PHONE, E.DEPT_ID, V.REASON, U.UNIT_NAME FROM EMPLOYEE E, VACATION V, DEPARTMENT D, UNIT U
	WHERE E.EMP_ID = V.EMP_ID AND D.DEPT_ID = E.DEPT_ID AND U.UNIT_ID = D.UNIT_ID 
    AND V.REASON = '두통' AND D.DEPT_NAME = '영업';

SELECT E.EMP_NAME, E.PHONE, E.DEPT_ID, V.REASON, U.UNIT_NAME 
FROM EMPLOYEE E INNER JOIN VACATION V INNER JOIN DEPARTMENT D INNER JOIN UNIT U
	ON E.EMP_ID = V.EMP_ID AND D.DEPT_ID = E.DEPT_ID AND U.UNIT_ID = D.UNIT_ID 
...
-- ANSI SQL : LEFT OUTER JOINT, RIGHT OUTER JOIN
SELECT * FROM SUBJECT;
SELECT * FROM PROFESSOR;
SELECT * 
	FROM SUBJECT S LEFT OUTER JOIN PROFESSOR P -- LEFT로 인해서 SUBJECT의 내용들이 다 나옴(가리키는 방향이 기준 테이블이 됨)
    ON S.SID = P.SID;
...
SELECT * FROM DEPARTMENT;
SELECT * FROM UNIT;
SELECT * FROM DEPARTMENT D LEFT OUTER JOIN UNIT U
	ON D.UNIT_ID = U.UNIT_ID ORDER BY D.UNIT_ID;
...
-- 학생들이 선택하지 않은 과목을 조회
SELECT * FROM SUBJECT SUB LEFT OUTER JOIN STUDENT STU
	ON SUB.SID = STU.SID
...
-- 홍길동 사원이 관리하는 모든 사원들을 조회
SELECT *
	FROM EMP E1, EMP E2
    WHERE E1.EMP_ID = E2.MGR AND E2.MGR = 'S0001';
...
SELECT E.EMP_NAME, M.EMP_NAME, E.EMP_ID, E.EMP_NAME, E.HIRE_DATE, E.SALARY, D.DEPT_ID, D.DEPT_NAME
...
-- 제외되는 데이터까지 조회하기 위해서는 OUTER JOIN이 필요함.
SELECT *
	FROM EMP M LEFT OUTER JOIN EMP E
    ON M.MGR = E.EMP_ID
    WHERE M.MGR IS NULL;

-- 홍길동 사원이 관리하는 사원들의 매니저사원아이디, 매니저사원명
-- 연봉, 매니저가 관리하는 사번, 매니저가 관리하는 사원명 조회
-- SELECT E.EMP_ID, E.EMP_NAME, E.SALARY, M.EMP_ID, M.EMP_NAME
SELECT *
...
SELECT  V.VACATION_ID, V.EMP_ID, V.BEGIN_DATE, V.REASON -- 컬럼리스트는 컬럼명을 정확하게 작성하는 것을 권장, 개발 시
...
-- 제3본부에 속한 부서들을 모두 출력
SELECT * FROM UNIT WHERE UNIT_NAME = '제3본부';    
SELECT * FROM DEPARTMENT
...
SELECT * FROM EMPLOYEE 
	WHERE DEPT_ID IN (SELECT DEPT_ID FROM DEPARTMENT WHERE DEPT_NAME = '정보시스템')
...
-- 출력행 포함
SELECT ROW_NUMBER() OVER(ORDER BY SALARY DESC) AS 'NO', EMP_ID, EMP_NAME, SALARY, DEPT_ID
	FROM EMPLOYEE;
    
-- 사원별 휴가사용 일수를 그룹핑하여, 사원아이디, 사원명, 입사일, 연봉, 휴가사용일수를 조회해주세요.
-- 휴가사용 일수를 구하는 인라인뷰와 사원테이블을 조인
SELECT * FROM VACATION WHERE EMP_ID ();

SELECT EMP_ID, SUM(DURATION) VCOUNT
	FROM VACATION GROUP BY EMP_ID;

SELECT ROW_NUMBER() OVER(ORDER BY VCOUNT DESC) AS NO,
E.EMP_ID, E.EMP_NAME, E.HIRE_DATE, CONCAT(FORMAT(E.SALARY, 0), '만원') AS SALARY, V.VCOUNT
	FROM EMPLOYEE E, 
		(SELECT EMP_ID, SUM(DURATION) VCOUNT
		FROM VACATION GROUP BY EMP_ID) V -- 인라인뷰의 별칭은 반드시 정의!!
	WHERE E.EMP_ID = V.EMP_ID;
